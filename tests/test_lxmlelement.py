...
class TestLxmlElement:
    ...
    def test_get_root(self, load_schema_from_file):
        ...
        root = XsdElement.get_root(element)
        assert root is not None

    def test_get_source_line(self, load_schema_from_file):
        element = load_schema_from_file.get_root()
        line = XsdElement.get_source_line(element)
        assert line == 1
    ...
    def test_get_children(self, load_schema_from_file):
        element = load_schema_from_file.get_root()
        children = XsdElement.get_children(element)
        assert len(children) == 3655

    def test_get_all_attributes(self, load_schema_from_file):
        element = load_schema_from_file.get_root()
    ...
    def test_get_localname(self, load_schema_from_file):
        element = load_schema_from_file.get_root()
        name = XsdElement.get_localname(element)
        assert name == "schema"

    def test_unique_tag(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        vessel = XsdElement.find_all_children_with_attribute_value(
            root, "element", "name", "Vessel"
        )[0]
        name = XsdElement.unique_tag(vessel)
    ...
    def test_get_name(self, load_schema_from_file):
        ...
        name = XsdElement.get_name(vessel)
        assert name == "Vessel"

    def test_get_use(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        panel = XsdElement.find_all_children_with_attribute_value(
            root, name="complexType", attrib_value="Panel_T", attrib_name="name"
        )
        if len(panel) > 0:
            panel = panel[0]
            attrib = XsdElement.find_attributes(panel)
            if len(attrib) > 0:
                use = XsdElement.get_use(attrib[0])
                assert use == "req."
            else:
                assert False
        else:
    ...
    def test_is_substitution_group(self, load_schema_from_file):
        ...
        assert XsdElement.is_substitution_group(vessel) is True

    def test_is_abstract(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        element = XsdElement.find_all_children_with_attribute_value(
            root, "element", "name", "Curve3D"
        )[0]
        assert XsdElement.is_abstract(element) is True
    ...
    def test_get_substitution_group(self, load_schema_from_file):
        ...
        assert XsdElement.get_substitution_group(vessel) == "ocx:Form"
    ...
    def test_find_all_children_with_name(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        children = XsdElement.find_all_children_with_name(root, "complexType")
    ...
    def test_find_child_with_name(self, load_schema_from_file):
        ...
        child = XsdElement.find_child_with_name(vessel, "annotation")
        assert len(child) == 1

    def test_find_attributes(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        attributes = XsdElement.find_attributes(root)
        assert len(attributes) == 155

    def test_find_attribute_groups(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        groups = XsdElement.find_attribute_groups(root)
    ...
    def test_has_child_with_name(self, load_schema_from_file):
        ...
        assert XsdElement.has_child_with_name(vessel, "annotation") is True
    ...
    def test_find_all_children_with_attribute_value(self, load_schema_from_file):
        ...
        root = load_schema_from_file.get_root()
        child = XsdElement.find_all_children_with_attribute_value(
    ...
    def test_find_all_children_with_name_and_attribute(self, load_schema_from_file):
        root = load_schema_from_file.get_root()
        children = XsdElement.find_all_children_with_name_and_attribute(
            root, "element", "name"
        )
